...
$sql = $con->prepare("SELECT id, nombre, precio FROM desayunos WHERE activo=1");
$sql->execute();
...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        ...
        if ($sql->fetchColumn() > 0) {
            ...
            $sql = $con->prepare("SELECT nombre, descriocion, precio, descuento FROM desayunos WHERE id=? AND
            activo=1 LIMIT 1");
            $sql->execute([$id]);
            ...
            if (file_exists($dir_images)) {
                ...
                while (($archivo = $dir->read()) != false) {
                    if ($archivo != 'desayuno.png' && (strpos($archivo, 'png') || strpos($archivo, 'png'))) {
